...
// アニメーションさせたい要素に.fadeIn-from-bottomを付与すればOK
// fadeIn-from-bottom用CSSを追加するのは忘れずに

// アニメーション対象要素の取得
const elms = document.querySelectorAll(".fadeIn-from-bottom");

// スクロールに合わせて.fade-from-bottom-animationを付与
const animation = (entries, observer) => {
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      entry.target.classList.add("fadeIn-from-bottom-animation");
